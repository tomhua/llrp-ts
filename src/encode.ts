 ...
 */
export const encodeMessage: Function = function (llrpMessage: LlrpMessage): Buffer {
    // Create the buffer, should be the length of the message.
    // We will not account for slicing the data.
    const buffer: Buffer = new Buffer(llrpMessage.getLength());
    ...
    if (llrpMessage.parameter.value.length !== 0) llrpMessage.parameter.value.copy(buffer, 10); // copy the parameter.
 ...
 * @return {Buffer}        writable buffer object.
 */
export const encodeParameter: Function = function (llrpParameter: LlrpParameter): Buffer {
